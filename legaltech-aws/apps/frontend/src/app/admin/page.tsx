...
const roleColors: Record<string, string> = {
  admin: "bg-brand-blue/10 text-brand-blue-light border-brand-blue/20",
  analyst: "bg-brand-teal/10 text-brand-teal-light border-brand-teal/20",
  client: "bg-slate-500/10 text-slate-300 border-slate-500/20",
  viewer: "bg-slate-600/10 text-slate-400 border-slate-600/20",
            ...
            return (
              <Card key={m.label}>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div
                    className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border ${m.bg}`}
                  >
                  ...
                  <div className="flex flex-col items-end gap-1">
                    <span
                      className={`inline-flex items-center rounded border px-2 py-0.5 text-[10px] font-semibold ${
                        roleColors[user.role] ?? "bg-slate-500/10 text-slate-300 border-slate-500/20"
                      }`}
                    >
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-brand-blue/10 border border-brand-blue/20">
                        <Building2 className="text-brand-blue-light" size={16} />
                      </div>
                      <div>
                        <p className="text-xs font-semibold text-slate-100">
                          {org.name}
            ...
            <div className="space-y-3">
              {mockAuditLogs.map((log) => (
                <div
                  ...
                  key={log.id}
                >
                  <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-brand-blue/10 border border-brand-blue/20">
                    <Shield className="text-brand-blue" size={12} />
                  </div>
                  <div className="min-w-0 flex-1">
                ...
                >
                  <span
                    className={`inline-flex items-center rounded border px-2.5 py-1 text-xs font-semibold shrink-0 ${
                      roleColors[item.role] ?? "bg-slate-500/10 text-slate-300 border-slate-500/20"
                    }`}
                  >
