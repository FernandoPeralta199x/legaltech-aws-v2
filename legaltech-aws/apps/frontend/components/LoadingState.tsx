...
export function LoadingState({
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-card-rest dark:border-slate-800 dark:bg-slate-900">
        ...
        {Array.from({ length: rows }).map((_, index) => (
          <div className="flex items-center gap-3" key={index}>
            ...
            <div className="flex-1 space-y-2">
              <div className={cn("h-2.5 rounded-full bg-slate-200 dark:bg-slate-700", index === 0 ? "w-2/3" : "w-1/2")} />
              <div className="h-2 rounded-full bg-slate-100 w-4/5 dark:bg-slate-800" />
            </div>
          </div>
        ))}
      </div>
